...
class MathematicsProfile(BASE):
    """
    class base of table for database with questions for mathematics subject with advanced difficulty.
    """
    __tablename__: str = "mathematics_profile"
...
class MathematicsProfileDB(DataBase):
    """
    Class creates or connects to database with questions for tests with mathematics subject. Class can create new question
    in database.
    """
    def __init__(self, db_name: str = "mathematics_profile_db"):
    ...
    def add_question(self, *, q_number: int, q_title: str, q_text: str, school_class: str,
        ...
        question: MathematicsProfile = MathematicsProfile(
            q_number=q_number,
            q_title=q_title,
            q_text=q_text,
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: MathematicsProfile = self.session.query(MathematicsProfile).get(question_id)
...
if __name__ == '__main__':
    _question = MathematicsProfileDB("../database/mathematics_db")
    ...
    for question_num in range(17):
        print(_question.session.query(MathematicsProfile).all())
