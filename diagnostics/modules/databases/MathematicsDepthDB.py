...
class MathematicsDepth(BASE):
    """
    class base of table for database with questions for mathematics in-depth subject.
    """
    __tablename__: str = "mathematics_depth"
...
class MathematicsDepthDB(DataBase):
    """
    Class creates or connects to database with questions for tests with mathematics in-depth subject. Class can create new question
    in database.
    """
    def __init__(self, db_name: str = "mathematics_depth_db"):
    ...
    def add_question(self, *, q_number: int, q_title: str, q_text: str, school_class: str,
        ...
        question: MathematicsDepth = MathematicsDepth(
            q_number=q_number,
            q_title=q_title,
            q_text=q_text,
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: MathematicsDepth = self.session.query(MathematicsDepth).get(question_id)
...
if __name__ == '__main__':
    _question = MathematicsDepthDB("../database/mathematics_depth_db")
    ...
    for question_num in range(17):
        print(_question.session.query(MathematicsDepth).all())
