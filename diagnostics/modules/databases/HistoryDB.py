...
class History(BASE):
    """
    class base of table for database with questions for History subject.
    """
...
class HistoryDB(DataBase):
    """
    Class creates or connects to database with questions for tests with History subject. Class can create new question
    in database.
    """
    def __init__(self, db_name: str = "history_db"):
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: History = self.session.query(History).get(question_id)
...
if __name__ == '__main__':
    ...
    for question_num in range(21):
        print(_question.session.query(History).all()[question_num].question_name)
