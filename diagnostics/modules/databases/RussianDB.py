...
class Russian(BASE):
    """
    class base of table for database with questions for Russian subject.
    """
    __tablename__: str = "russian"
...
class RussianDB(DataBase):
    """
    Class creates or connects to database with questions for tests with Russian subject. Class can create new question
    in database.
    """
    def __init__(self, db_name: str = "russian_db"):
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: Russian = self.session.query(Russian).get(question_id)
...
if __name__ == '__main__':
    ...
    for question_num in range(21):
        print(_question.session.query(Russian).all()[question_num].question_name)
