...
class Literature(BASE):
    """
    class base of table for database with questions for Literature subject.
    """
    __tablename__: str = "literature"
...
class LiteratureDB(DataBase):
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: Literature = self.session.query(Literature).get(question_id)
...
if __name__ == '__main__':
    ...
    for question_num in range(21):
        print(_question.session.query(Literature).all()[question_num].q_title)
