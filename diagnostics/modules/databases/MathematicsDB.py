...
class Mathematics(BASE):
    """
    class base of table for database with questions for mathematics subject.
    """
...
class MathematicsDB(DataBase):
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: Mathematics = self.session.query(Mathematics).get(question_id)
...
if __name__ == '__main__':
    _question = MathematicsDB("../database/mathematics_db")
    ...
    for question_num in range(17):
        print(_question.session.query(Mathematics).all())
