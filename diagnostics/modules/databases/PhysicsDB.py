...
class Physics(BASE):
    """
    class base of table for database with questions for Physics subject.
    """
    __tablename__: str = "physics"
...
class PhysicsDB(DataBase):
    """
    Class creates or connects to database with questions for tests with Physics subject. Class can create new question
    in database.
    """
    def __init__(self, db_name: str = "physics_db"):
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: Physics = self.session.query(Physics).get(question_id)
...
if __name__ == '__main__':
    ...
    for question_num in range(21):
        print(_question.session.query(Physics).all()[question_num].question_name)
