...
class Chemistry(BASE):
    """
    class base of table for database with questions for chemistry subject.
    """
    __tablename__: str = "chemistry"
...
class ChemistryDB(DataBase):
    """
    Class creates or connects to database with questions for tests with chemistry subject. Class can create new question
    in database.
    """
    ...
    def change_question(self, *, question_id: str, data: dict[str, str]) -> None:
        question: Chemistry = self.session.query(Chemistry).get(question_id)
...
if __name__ == '__main__':
    _question = ChemistryDB()
    for num in range(20):
        _question.add_question(
            q_number=num,
            q_title=f"Problem_{num}",
            q_text=f"Solute_problem_{num}",
            q_answer_variants="one , two, three, four",
            school_class="9-Б",
            q_image="",
            q_right_answer="two"
        )
    for question_num in range(21):
        print(_question.session.query(Chemistry).all()[question_num].q_title)
